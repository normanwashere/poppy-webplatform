...
Now, open your poppyfulllogged2.html file.

Important: Copy everything inside the <body> tags from poppyfulllogged2.html.

Paste this copied content into src/App.svelte.

Before the <body> content, add the following Svelte script and style tags:
...
<script>
    ...
    // All your JavaScript logic from the <script> tag in poppyfulllogged2.html goes here.
    // Make sure to remove the script tags themselves, just the content.

    // --- TEMPLATES FOR EACH PAGE ---
    // These will need to be refactored into separate Svelte components later for better practice.
    // For now, we'll keep them as strings for direct migration.
    let pageTemplates = { // Changed to 'let' as it's modified by the script below
        login: `
...
<button class="delete-rule-btn clay-button !p-2" data-ruleid="rule.id"><idata−lucide="trash−2"class="h−4w−4"></i></button></div><divclass="gridgrid−cols−2sm:grid−cols−4gap−4text−center"><divclass="clay−insetp−3"><spanclass="blocktext−smfont−semiboldtext−gray−500uppercase">LiveHours</span><spanclass="blocktext−xlfont−bold">{rule.liveHours}</span>
</div>
<div class="clay-inset p-3">
<span class="block text-sm font-semibold text-gray-500 uppercase">Branded</span>
<span class="block text-xl font-bold">rule.brandedItems</span></div><divclass="clay−insetp−3"><spanclass="blocktext−smfont−semiboldtext−gray−500uppercase">FreeSize</span><spanclass="blocktext−xlfont−bold">{rule.freeSize}</span>
</div>
<div class="clay-inset p-3">
...
`).join('');
    ...
    function renderUserList() {
        ...
        let userRows = mockUsers.filter(u => u.role !== 'admin').map((user, index) => `
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 <span class="math-inline">\{index % 2 \!\=\= 0 ? 'bg\-\[rgba\(0,0,0,0\.04\)\]' \: ''\}"\>
<div class="text-center sm:text-left flex-grow cursor-pointer view-user-details" data-userid="{user.id}">
<p class="font-bold">user.name</p><pclass="text−smtext−gray−600">{user.email}</p>
</div>
    ...
    function runDashboardLogic(role) {
        ...
        function renderBonusCards(performanceData, filterRange) {
            ...
            bonusCardsContainer.innerHTML = activeRules.map(rule => {
                ...
                return `
                    <div class="clay-card p-4 cursor-pointer <span class="math-inline">\{\!isApplicableInFilter ? 'opacity\-50' \: ''\}" data\-ruleid\="</span>{rule.id}">
                         <div class="flex justify-between items-start">
                            <h4 class="font-playfair font-bold text-lg mb-2">${rule.name}</h4>
                            <div class="tooltip-container">
                                <i data-lucide="info" class="h-5 w-5 text-gray-500"></i>
                                <span class="tooltip-text">Targets: ${rule.liveHours} hrs, ${rule.brandedItems} branded, <span class="math-inline">\{rule\.freeSize\} free\-size</span\>
</div>
</div>
...
`;
        ...
        function update() {
            ...
            activeAndApplicableRules.forEach(rule => {
                ...
                if (metrics.duration >= rule.liveHours && // Corrected variable name from liveHours to duration
                    metrics.branded >= rule.brandedItems && // Corrected variable name from brandedItems to branded
                    metrics.freeSize >= rule.freeSize) {   // Corrected variable name from freeSize to freeSize
        ...
        function renderDashboardTable() {
             ...
             paginatedData.forEach((item, index) => {
                ...
                row.innerHTML = `<td class="p-4 whitespace-nowrap">${itemDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</td> ${role === 'admin' ? `<td class="p-4 whitespace-nowrap">${item.seller}</td>` : ''} <td class="p-4 whitespace-nowrap"><span class="math-inline">\{duration\.toFixed\(2\)\} hrs</td\> <td class\="p\-4 whitespace\-nowrap"\></span>{item.branded}</td> <td class="p-4 whitespace-nowrap">${item.freeSize}</td>`;
        ...
        function renderDashboardPagination() {
            ...
            pageInfo.innerHTML = `Page <strong><span class="math-inline">\{dashboardCurrentPage\}</strong\> of <strong\></span>{totalPages}</strong>`;
        ...
        if (role === 'admin' && sellerFilter) {
            ...
            sellerFilter.innerHTML = uniqueSellers.map(s => `<option value="<span class="math-inline">\{s\}"\></span>{s}</option>`).join('');
...
<style>
    /* All your CSS from the <style> tag in poppyfulllogged2.html goes here.
       Make sure to remove the style tags themselves, just the content. */
...
<body class="p-4 sm:p-6 md:p-8">
    ...
    <!-- Main Navigation (shown after login) -->
    <header id="main-header" class="hidden clay-card p-4 mb-8 sticky top-4 z-40">
        <nav class="flex flex-wrap items-center justify-between gap-4">
            <div id="nav-links" class="flex items-center gap-2">
                <!-- Nav links are populated by JS -->
            </div>
            <div class="relative">
                <button id="profile-button" class="flex items-center gap-3">
                    <div class="bg-white p-2 rounded-full shadow-inner">
    ...
    <!-- Modals (kept globally available) -->
     <div id="delete-confirmation-modal" class="hidden fixed inset-0 bg-black bg-opacity-30 backdrop-blur-sm flex items-center justify-center p-4 z-50">
        <div class="clay-card w-full max-w-md p-6 sm:p-8">
            <h2 class="font-playfair text-2xl font-bold text-gray-700 mb-4">Confirm Deletion</h2>
            <p class="text-gray-600 mb-6">To delete this bonus rule, please enter your password to confirm.</p>
    ...
    <script>
        ...
        function runDashboardLogic(role) {
            ...
            function update() {
                ...
                activeAndApplicableRules.forEach(rule => {
                     if (metrics.liveHours >= rule.liveHours && 
                         metrics.brandedItems >= rule.brandedItems && 
